...
class NegativeSampler(BaseSampler):
    ...
    def _generate_random(self, count: int, start: int, stop: int) -> List[int]:
        ...
        return np.random.choice(numbers, size=count, replace=False).tolist()


    def sample(self, channel: object) -> List[SampleSeq]:
        if len(self._sample_point_list) == 0:
            self.__init_sample_points(self._k, len(channel.length))
        return [super()._sample_with_event(index, channel) for index in self._sample_point_list]
